...
# Data Processing
Dentro do [ecossistema do Querido Diário](https://github.com/okfn-brasil/querido-diario-comunidade/blob/main/.github/CONTRIBUTING.md#ecossistema-do-querido-di%C3%A1rio), este repositório é o responsável pelo **processamento do texto dos diários oficiais raspados**.
...
Por meio de um terminal aberto no diretório raiz do repositório, use a sequência de comandos a seguir para configuração: 
```
make build
make setup
```

O comando `make build` só precisa ser executado na primeira vez que interagir com o repositório. 
...
Para ver os diários oficiais processados, use a query: 
```
select processed, count(1) from gazettes g group by processed;
```

Você pode pesquisar usando o ElasticSearch:
```
