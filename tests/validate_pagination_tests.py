...
def validate_pagination_code():
    ...
    code_file = (
        Path(__file__).parent.parent / "tasks" / "list_gazettes_to_be_processed.py"
    )
    ...
    if (
        "database.select(command, params)" in code
        or "database.select(command, data)" in code
    ):
    ...
    if (
        'logging.debug(f"Processing page' in code
        or 'logging.debug(f"Processing page' in code
    ):
