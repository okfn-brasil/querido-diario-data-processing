...
def extract_text_pending_gazettes(
    database, storage, index, text_extractor, territories=None
):
    ...
    pending_gazettes = (
        database.get_pending_gazettes()
        if hasattr(database, "get_pending_gazettes")
        else []
    )
    territories = territories or []

    return extract_text_from_gazettes(
        pending_gazettes, territories, database, storage, index, text_extractor
    )


def upload_gazette_raw_text(storage, gazette_id, content, file_key=None):
