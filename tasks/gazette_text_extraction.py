...
def try_process_gazette_file(
    ...
    try:
        ...
        if error_code == "404":
            ...
            logging.error(
                f"Gazette ID: {gazette.get('id')}, Checksum: {gazette.get('file_checksum')}"
            )
    ...
    try:
        ...
        use_relative_paths = os.environ.get("USE_RELATIVE_FILE_PATHS", "false").lower() == "true"
    ...
    try:
        ...
        if os.path.exists(gazette_file):
            try:
                ...
                logging.warning(
                    f"Failed to cleanup file {gazette_file}: {cleanup_error}"
                )
        raise e
