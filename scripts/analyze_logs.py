...
def print_report(stats):
    ...
    print(
        f"Requisições falhadas: {stats['tika']['requests'] - stats['tika']['responses']}"
    )
    ...
    if stats["tika"]["errors"]:
        ...
        for error_type, count in sorted(
            stats["tika"]["errors"].items(), key=lambda x: x[1], reverse=True
        ):
    ...
    if stats["opensearch"]["operations"]:
        ...
        for op_type, count in sorted(
            stats["opensearch"]["operations"].items(), key=lambda x: x[1], reverse=True
        ):
    ...
    if stats["opensearch"]["errors"]:
        print("\n❌ Erros do OpenSearch:")
        for error_type, count in sorted(
            stats["opensearch"]["errors"].items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  - {error_type}: {count} ocorrências")
    ...
    print("-" * 70)
    total_issues = (
        stats["connection_issues"]["chunked_encoding"]
    ...
    if total_issues > 0:
        ...
        print("\nDetalhes:")
        print(
            f"  - ChunkedEncodingError: {stats['connection_issues']['chunked_encoding']}"
        )
        ...
        if stats["connection_issues"]["chunked_encoding"] > 0:
            print("  • ChunkedEncodingError detectado - Possíveis causas:")
            print("    - Tika server encerrando conexão prematuramente")
            print("    - Timeout na rede entre containers")
            print("    - Arquivo muito grande causando timeout")
            print("    - Problema de memória no Tika")

        if stats["connection_issues"]["incomplete_reads"] > 0:
            print("  • IncompleteRead detectado - Possíveis causas:")
            print("    - Resposta do Tika cortada")
            print("    - Problema de rede intermitente")
            print("    - Tika crashando durante processamento")
    else:
    ...
    tika_success_rate = (
        (stats["tika"]["responses"] / stats["tika"]["requests"] * 100)
        if stats["tika"]["requests"] > 0
        else 0
    )
