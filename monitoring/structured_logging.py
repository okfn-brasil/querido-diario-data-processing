...
class ConnectionMonitor:
    ...
    def record_tika_request(self, success: bool, duration_ms: float, error_type: Optional[str] = None):
        ...
        if success:
            ...
            if error_type:
                self.stats['tika']['errors'][error_type] = self.stats['tika']['errors'].get(error_type, 0) + 1
    
    def record_opensearch_operation(self, success: bool, duration_ms: float, error_type: Optional[str] = None):
        ...
        if success:
            ...
            if error_type:
                self.stats['opensearch']['errors'][error_type] = self.stats['opensearch']['errors'].get(error_type, 0) + 1
    ...
    def print_summary(self):
        ...
        if self.stats['tika']['total_requests'] > 0:
            avg_duration = self.stats['tika']['total_duration_ms'] / self.stats['tika']['total_requests']
        ...
        if self.stats['opensearch']['total_operations'] > 0:
            avg_duration = self.stats['opensearch']['total_duration_ms'] / self.stats['opensearch']['total_operations']
            print(f"  Duração média: {avg_duration:.2f}ms")
...
def log_tika_request(filepath: str, file_size: int, content_type: str, url: str) -> None:
    """
...
def log_tika_response(filepath: str, duration_ms: float, response_size: int, status_code: int) -> None:
    """
...
def log_tika_error(filepath: str, error_type: str, error_message: str, duration_ms: float, 
                   file_size: Optional[int] = None, status_code: Optional[int] = None) -> None:
    """
    ...
    # Registra no monitor
    _global_monitor.record_tika_request(success=False, duration_ms=duration_ms, error_type=error_type)


# Funções de logging estruturado para OpenSearch
def log_opensearch_operation(operation: str, index_name: str, duration_ms: float, 
                             document_id: Optional[str] = None, success: bool = True,
                             document_size: Optional[int] = None) -> None:
    """
    ...
    # Registra no monitor
    _global_monitor.record_opensearch_operation(success=success, duration_ms=duration_ms)


def log_opensearch_error(operation: str, index_name: str, error_type: str, 
    ...
    # Registra no monitor
    _global_monitor.record_opensearch_operation(success=False, duration_ms=duration_ms, error_type=error_type)
...
def monitor_tika_call(func):
    ...
    def wrapper(self, filepath: str) -> str:
        ...
        content_type = self._get_file_type(filepath) if hasattr(self, '_get_file_type') else 'unknown'
...
def monitor_opensearch_call(operation: str):
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, **kwargs):
            index_name = kwargs.get('index', self._default_index if hasattr(self, '_default_index') else 'unknown')
